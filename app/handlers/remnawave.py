...
async def cb_create_user(callback: CallbackQuery, user: TelegramUser):
...
async def cb_get_key(callback: CallbackQuery, repo: RequestsRepo):

    subscription = await repo.subscription.get_by_tg_user_id(callback.from_user.id)
    ...
    expires_at = subscription.subscription_date
    ...
    await callback.message.answer(
        f"Ваша ссылка на подписку: {vpn_key}\\nПодписка действует до: {expires_str}"
    )
    await callback.answer()
